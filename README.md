...
This command will help you to invoke the lambda function and it will store the responses of each payload file to post-deploy-responses/$function_name directory.This command will be same but only the environment will changes based on the environment we are invoking lambda function.

In the post conditions we have two steps i.e; on success we have to sent an email that the job is geeting succedded and on failure we must have to sent an email that the job has been failed and also deleting the job workspace whenever the job is failed.And finally the docker container will stops here and exits. 


**artifacts**

This will done using an another node which is not a part of declarative pipeline.In this artifacts stage we are copying postdeploy-responses which we got during the lambda-invoke from our job workspace to jenkins home directory.As we know that lambda-invoke should take place only when user wants to do `PostDeploymentTest` or `UpdateCode` so this artifacts stage also should be done only in that specific cases.Here also we are running a bash script file(i.e; post-deploy-responses.sh) to copy those response files.In the script file we are checking a condition that lambda function name folder is exists or not.If the function named folder is present under post-deploy-responses folder then we can directly copy the responses files,If the folder is not present then we will create a folder with function name and we will change its ownership and permissions of the file and then we copy the repsonse files from job worksapce to jenkins home directory.Finally after all the stages we are deleting the job workspace.
